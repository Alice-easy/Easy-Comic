...
class MangaRepositoryImpl(
    ...
    override fun getMangaByStatus(status: DomainReadingStatus): Flow<List<Manga>> {
        return mangaDao.getMangaByStatus(status.toDataReadingStatus()).map { entities ->
            entities.map { it.toDomain() }
        }
    }
    ...
    override suspend fun updateReadingProgress(
        mangaId: Long,
        currentPage: Int,
        status: DomainReadingStatus
    ) {
        mangaDao.updateReadingProgress(mangaId, currentPage, status.toDataReadingStatus())
    }
...
private fun MangaEntity.toDomain(): Manga {
    return Manga(
        ...
        readingStatus = readingStatus.toDomainReadingStatus(),
...
private fun Manga.toEntity(): MangaEntity {
    return MangaEntity(
        ...
        rating = rating,
        isFavorite = isFavorite,
        readingStatus = readingStatus.toDataReadingStatus(),
...
/**
 * Domain ReadingStatus 转换为 Data ReadingStatus
 */
private fun DomainReadingStatus.toDataReadingStatus(): DataReadingStatus {
    return when (this) {
        DomainReadingStatus.UNREAD -> DataReadingStatus.UNREAD
        DomainReadingStatus.READING -> DataReadingStatus.READING
        DomainReadingStatus.COMPLETED -> DataReadingStatus.COMPLETED
        DomainReadingStatus.PAUSED -> DataReadingStatus.PAUSED
        DomainReadingStatus.DROPPED -> DataReadingStatus.DROPPED
    }
}

/**
 * Data ReadingStatus 转换为 Domain ReadingStatus
 */
private fun DataReadingStatus.toDomainReadingStatus(): DomainReadingStatus {
    return when (this) {
        DataReadingStatus.UNREAD -> DomainReadingStatus.UNREAD
        DataReadingStatus.READING -> DomainReadingStatus.READING
        DataReadingStatus.COMPLETED -> DomainReadingStatus.COMPLETED
        DataReadingStatus.PAUSED -> DomainReadingStatus.PAUSED
        DataReadingStatus.DROPPED -> DomainReadingStatus.DROPPED
    }
}